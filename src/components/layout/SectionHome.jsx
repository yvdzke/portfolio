import { FaGithub, FaLinkedinIn, FaInstagram } from "react-icons/fa";
import Button from "../button/index.jsx";

const SectionHome = () => {
  return (
    <section className="home">
      {/* =============== Home Wrapper ===============*/}
      <div className="home-wrapper bg-[linear-gradient(90deg,#F0EBE3_56%,#FFFFFF_94%)] h-screen flex items-center justify-center">
        {/* =============== Home Container ===============*/}
        <div className="home-container flex gap-[30px] px-[8px] py-[29px] items-center justify-center w-[1116px] h-[586px] ">
          {/* =============== Intro Banner ===============*/}
          <div className="intro-banner border  flex flex-col px-[9px] gap-[20px] py-[46px] w-[578px] h-[528px] border-black ">
            {/* Hello */}
            <p className="className font-jost font-bold text-[14px] ">
              HELLO, <span className="text-[#1FB7CA]">MY NAME IS</span>
            </p>
            {/* Name */}
            <div className="flex flex-col">
              <p className="className font-jost font-bold text-[#1FB7CA] [-webkit-text-stroke:2px_black] [paint-order:stroke_fill] text-[48px]">
                YUDA <span className="text-white">PRADANA</span>
              </p>
              <div className="flex flex-raw gap-[11px] items-center">
                <p className="font-jost text-[16px] font-medium">I AM</p>
                <p className="font-hand text-[25px] font-medium">
                  Web Developer
                </p>
              </div>
            </div>
            {/* Description */}
            <p className="max-w-[520px] text-[14px] text-justify font-jost leading-relaxed">
              Graduated Full-Stack Web Developer bootcamp program from
              harisenin.com for deepening skills in Web Development. Explore my
              latest projects, showcasing my expertise in web development.
            </p>
            {/* Link */}
            <div className="flex flex-raw gap-[11px]">
              <a
                target="blank"
                className="hover:text-[#1FB7CA]"
                href="https://github.com/yvdzke"
              >
                <FaGithub size={19} />
              </a>
              <a target="blank" href="https://github.com/yvdzke">
                <FaLinkedinIn className="hover:text-[#1FB7CA]" size={19} />
              </a>
              <a
                target="blank"
                className="hover:text-[#1FB7CA]"
                href="https://github.com/yvdzke"
              >
                <FaInstagram size={19} />
              </a>
            </div>
            {/* Button Download CV + My Skills */}
            <div className="flex flex-raw items-center gap-[20px]">
              {/* Button + Line */}
              <div className="flex flex-raw items-center">
                <Button
                  variant="relative overflow-hidden bg-white isolate border border-black text-black font-jost font-medium text-[12px] w-[140px] h-[40px] items-center rounded-full transition-colors duration-500  
before:absolute before:inset-0 before:-z-10 before:bg-[#1FB7CA] before:rounded-full before:scale-0 before:transition-transform before:duration-500 before:content-[''] 
hover:before:scale-95"
                >
                  DOWNLOAD CV
                </Button>
                <div className="bg-black w-10 h-[2px]"></div>
              </div>
              <p className="font-jost font-semibold text-[12px] cursor-pointer hover:text-[#1FB7CA]">
                MY SKILLS
              </p>
            </div>
          </div>
          {/* =============== Profile Banner ===============*/}
          <div className="profile-banner rounded-tl-[200px] relative  rounded-br-[200px] w-[574px] h-[528px] bg-[#1FB7CA]">
            {/* Miku Profile */}
            <img
              className="rounded-br-[215px] mt-10 object-cover"
              src="https://res.cloudinary.com/dvym5vxsw/image/upload/v1770286304/Hatsune-Miku-PNG-File_t309n0.png"
              alt=""
            />
            {/* Absolute Items */}
            <img
              className="absolute w-[180px] h-[180px] top-[-70px] right-[-60px] "
              src="https://res.cloudinary.com/dvym5vxsw/image/upload/v1770332237/shape-1-f115230c_nb2klm.png"
              alt=""
            />
            <img
              className="absolute w-[180px] h-[180px] bottom-[-70px] left-[-100px] "
              src="https://res.cloudinary.com/dvym5vxsw/image/upload/v1770332237/shape-1-f115230c_nb2klm.png"
              alt=""
            />
            {/* Shape Items */}
            <div className="absolute w-[170px] h-[58px]  bottom-[80px] border border-black left-[-30px] bg-white rounded-full">
              <div className="flex flex-raw items-center gap-[10px] px-[40px] py-[2px]">
                <p className="font-semibold text-[20px]">1+</p>
                <div className="text-center">
                  <p className="font-semibold text-[16px]">Bootcamp</p>
                  <p className="font-semibold text-[16px] text-[#1FB7CA]">
                    Harisenin
                  </p>
                </div>
              </div>
            </div>
            <div className="absolute w-[170px] h-[58px]  bottom-[30px] border border-black right-[-30px] bg-white rounded-full">
              <div className="flex flex-raw items-center gap-[10px] px-[40px] py-[2px]">
                <p className="font-semibold text-[20px]">1+</p>
                <div className="text-center">
                  <p className="font-semibold text-[16px]">Figma</p>
                  <p className="font-semibold text-[16px] text-[#1FB7CA]">
                    Design
                  </p>
                </div>
              </div>
            </div>
          </div>
        </div>
      </div>
    </section>
  );
};

export default SectionHome;
